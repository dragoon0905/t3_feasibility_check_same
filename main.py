...
################################
        ...
        saved_state_dict0 = torch.load("/data/seunan/t3_feasibility_check/pretrained/GTA5_150000.pth", map_location=self.device)
...
################################
        ...
        saved_state_dict1 = torch.load("/data/seunan/t3_feasibility_check_same/finetuning_tm/cityscapes.pth", map_location=self.device)
...
################################
        ...
        saved_state_dict2 = torch.load("/data/seunan/t3_feasibility_check_same/finetuning_tm/idd.pth", map_location=self.device)
...
################################ Pseudo1
                ...
                T = 0.9
                mask_1 = (maxpred3_1 > T) | ((argpred3_1==label_0_1) & (argpred3_1==label_1_1) & (argpred3_1==label_2_1))
                ignore_tensor = torch.ones(1).to(
                    self.device, dtype=torch.long) * self.ignore_index
                label_1 = torch.where(mask_1, label_3_1, ignore_tensor)
                mask_1 = (maxpred3_1 > T)
                label_1_basic=torch.where(mask_1, label_3_1, ignore_tensor)
...
################################ Pseudo2
                ...
                T = 0.9
                pred_c = (pseudo_outputs3_1 + pseudo_outputs3_2) / 2
                maxpred_c, argpred_c = torch.max(pred_c, dim=1)
                mask_2 = (maxpred3_2 > T) | ((label_3_2==label_0_2) & (label_3_2==label_1_2)  & (label_3_1==label_2_2)) | (maxpred3_1 > T) | ((label_3_1==label_0_1) & (label_3_1==label_1_1) & (label_3_1==label_2_1))
                ignore_tensor = torch.ones(1).to(
                    self.device, dtype=torch.long) * self.ignore_index
                label_2 = torch.where(mask_2, argpred_c, ignore_tensor)
                ...
                if self.epoch>=0 and batch_idx > 0 and batch_idx % 1000 == 0:
                 ...
                 # pseudo_ = torch.max(pseudo_, dim=1)[1]
                    ...
                    pseudo_ = np.asarray(pseudo_, dtype=np.uint8).squeeze(0)
                    output_col = colorize_mask(pseudo_)
                    output_col.save('/data/seunan/t3_feasibility_check_same/t3_pseudo_same/p2_%s_%s_color_gt.png' % (batch_idx,self.epoch))
                    #t1
                    
                    #t3 threshold 0.9
                    #pseudo3_2 = torch.max(pseudo3_2, dim=1)[1]
                    pseudo_ = label_1_basic[0].unsqueeze(0)
                    pseudo_ = pseudo_.detach().cpu().numpy()
                    pseudo_ = np.asarray(pseudo_, dtype=np.uint8).squeeze(0)
                    output_col = colorize_mask(pseudo_)
                    output_col.save('/data/seunan/t3_feasibility_check_same/t3_pseudo_same/p2_%s_%scolor_t3.png' % (batch_idx,self.epoch))
                    #best
                    pseudo_ = label_1[0].unsqueeze(0)
                    pseudo_ = pseudo_.detach().cpu().numpy()
                    pseudo_ = np.asarray(pseudo_, dtype=np.uint8).squeeze(0)
                    output_col = colorize_mask(pseudo_)
                    output_col.save('/data/seunan/t3_feasibility_check_same/t3_pseudo_same/p2_%s_%s_color_best.png' % (batch_idx,self.epoch))
################################
            ...
            # Validation  #
            ###############
            if self.epoch>=0 and batch_idx > 0 and batch_idx % 2000 == 0:
